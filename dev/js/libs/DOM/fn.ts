...
export const parent = (el: HTMLAnyElement): HTMLAnyElement => {
    return el.parentElement as HTMLAnyElement;
};
...
export const attr = (el: HTMLAnyElement, attr?: string, value?: string) => {
    ...
    el.setAttribute(attr as string, value);
};
...
export const closest = (
    ...
    while (
        !parent ||
        !parent.tagName ||
        !Array.prototype.includes.call(realSearch, parent)
    ) {
        if (parent === null) {
            break;
        }
...
export const each = (
    ...
    fn: (item: any, i: number, arr: any[] | NodeList | HTMLCollection) => void,
    breakOn?: number
) => {
...
export const ajax = async ({
    ...
    xhrParams,
    minTime = 0,
}: AjaxParams): Promise<XMLHttpRequest> => {
    return new Promise(async (resolve: any, reject: any) => {
        ...
        xhr.addEventListener("loadend", (res) => {
          ...
          if (target.status < 200 || target.status > 299) {
            ...
            if (siteUrl === "/checklist") {
              window.open("assets/files/checklist.pdf", "_blank");
            }
            if (!getXhr) {
              let endTime = Date.now();
              let diffTime = endTime - endTime;
              if (diffTime < minTime) {
                setTimeout(() => {
                  resolve(target, res);
                }, minTime - diffTime);
...
const getRequestMethod = (method, data): string => {
...
export const slideUp = (target: HTMLElement) => {
    ...
    setTimeout(() => {
        target.setAttribute(
            "style",
            "height: 0px; margin-top: 0px; margin-bottom: 0px; padding-top: 0px; padding-bottom: 0px;"
        );
        setTimeout(() => {
